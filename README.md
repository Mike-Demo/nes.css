...
The base published `nes.css` package is compatible with the newest versions of Chrome, Firefox, and Safari. The `/docs` demo in this repository has narrower expectations because it relies on a docs-specific stylesheet plus `position: sticky`, `backdrop-filter`, `scroll-behavior: smooth`, clipboard support with a legacy fallback, syntax highlighting, and Vue-powered interactions.
