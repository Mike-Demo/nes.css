...
The base published `nes.css` package is compatible with the newest versions of Chrome, Firefox, and Safari. The `/docs` demo in this repository uses a bundled docs CSS snapshot and has narrower expectations because it also relies on `position: sticky`, `backdrop-filter`, `scroll-behavior: smooth`, clipboard access, syntax highlighting, and Vue-powered interactions.
